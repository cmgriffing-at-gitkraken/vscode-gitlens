...
export class OtherAIProvider implements AIProvider<'otherai'> {
	...
	private get model(): OtherAIModels | null {
		return configuration.get('ai.experimental.otherai.model') || null;
	}

	private get url(): string {
		return configuration.get('ai.experimental.otherai.url') || 'https://api.otherai.com/v1/chat/completions';
	}

	private async getOrChooseModel(): Promise<OtherAIModels | undefined> {
	...
	async generateCommitMessage(diff: string, options?: { context?: string }): Promise<string | undefined> {
		...
		while (true) {
			...
			const request: OtherAIChatCompletionRequest = {
				model: model,
				messages: [
					{
						role: 'system',
...
Follow the user's instructions carefully, don't repeat yourself, don't include the code in the output, or make anything up!`,
			...
			if (diff.length > maxCodeCharacters) {
				void window.showWarningMessage(
					`The diff of the changes had to be truncated to ${maxCodeCharacters} characters to fit within the OtherAI's limits.`,
				);
			}

			const data: OtherAIChatCompletionResponse = await rsp.json();
...
Do not make any assumptions or invent details that are not supported by the code diff or the user-provided context.`,
	...
	private fetch(apiKey: string, request: OtherAIChatCompletionRequest) {
		const url = this.url;
...
async function getApiKey(storage: Storage): Promise<string | undefined> {
	let openaiApiKey = await storage.getSecret('gitlens.otherai.key');
	if (!openaiApiKey) {
		...
		try {
			const infoButton: QuickInputButton = {
				...
				tooltip: 'Open the OtherAI API Key Page',
			};
				...
				disposables.push(
					...
					input.onDidChangeValue(value => {
						if (value && !/(?:sk-)?[a-zA-Z0-9]{32,}/.test(value)) {
							input.validationMessage = 'Please enter a valid OtherAI API key';
							return;
						}
					...
					input.onDidTriggerButton(e => {
						if (e === infoButton) {
							void env.openExternal(Uri.parse('https://platform.otherai.com/account/api-keys'));
						}
					}),
				);

				input.password = true;
				input.title = 'Connect to OtherAI';
				input.placeholder = 'Please enter your OtherAI API key to use this feature';
				input.prompt = supportedInVSCodeVersion('input-prompt-links')
					? 'Enter your [OtherAI API Key](https://platform.otherai.com/account/api-keys "Get your OtherAI API key")'
					: 'Enter your OtherAI API Key';
		...
		void storage.storeSecret('gitlens.otherai.key', openaiApiKey);
	}

	return openaiApiKey;
}

export type OtherAIModels =
	| 'gpt-4-turbo-preview'
	| 'gpt-4-0125-preview'
	| 'gpt-4-1106-preview'
	| 'gpt-4'
...
interface OtherAIChatCompletionRequest {
	model: OtherAIModels;
...
interface OtherAIChatCompletionResponse {
	id: string;
